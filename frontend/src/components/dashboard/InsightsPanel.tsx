...
export default function InsightsPanel({ recommendedLocality }: InsightsPanelProps) {
  // Defensive fallbacks to maintain structural safety if data properties fluctuate
  const schools = recommendedLocality.amenities?.schools || ["Localized schooling blocks available"];
  const hospitals = recommendedLocality.amenities?.hospitals || ["Regional diagnostic centers close"];
  const transit = recommendedLocality.amenities?.metro_stations || ["Multi-modal road link options"];

  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      
      {/* COLUMN 1: Qualitative Pros & Cons Evaluation Layout */}
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl space-y-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Side-by-side contextual trade-offs for {recommendedLocality.name}</p>
        </div>

        <div className="space-y-4">
          {/* Positives Enclave Vector Rows */}
      ...
      {/* COLUMN 2: Infrastructure Layers & AI Consultant Wrap */}
      <div className="flex flex-col gap-6">
        
        {/* Sub-Panel A: Labeled Amenity Matrices */}
        ...
        {/* Sub-Panel B: Strategic Relocation Synthesis Anchor */}
